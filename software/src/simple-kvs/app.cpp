...
uint32_t app_thrd::do_rx() {
  ...
  rx_batch_size =
      rounded_size < APP_RX_BATCH_SIZE ? rounded_size : APP_RX_BATCH_SIZE;

  rx_batch_size = rx_batch_size < _worker_ring->n_empty_slots
                      ? rx_batch_size
                      : _worker_ring->n_empty_slots;

  if (rx_batch_size > 0) {
    n_rx =
        app_rx_batch(_rmtp, _rx_mbufs + _worker_ring->ring_head, rx_batch_size);

    if (n_rx > 0) {
      _worker_ring->ring_head =
          (_worker_ring->ring_head + n_rx) % WORKER_RING_SIZE;
...
void app_thrd::do_work() {
  ...
  if (_worker_ring->unprocessed_work_cnt > 0) {
    uint32_t work_size =
        get_proc_size(_worker_ring->ring_head, _worker_ring->work_head,
                      WORKER_RING_SIZE, APP_DO_WORK_THRESH);
...
#ifdef DBSUPPORT
      db_get(_db, (uint8_t *)(rmtp_h + 1) + 2, _rx_mbufs[ind]->pkt_len - 64 - 2,
             &val);
    ...
    _worker_ring->work_head =
        (_worker_ring->work_head + work_size) % WORKER_RING_SIZE;
  }
}
...
uint32_t app_thrd::do_tx() {
  ...
  uint32_t tx_batch_size =
      get_proc_size(_worker_ring->work_head, _worker_ring->ring_tail,
                    WORKER_RING_SIZE, APP_TX_BATCH_SIZE);

  if (tx_batch_size > 0) {
    uint32_t ind = _worker_ring->ring_tail;
    n_tx = app_tx_batch(_rmtp, _buf + ind, _buf_len + ind, _dst_qids + ind,
                        _is_ack + ind, tx_batch_size);

    if (n_tx > 0) {
      _worker_ring->ring_tail =
          (_worker_ring->ring_tail + n_tx) % WORKER_RING_SIZE;
